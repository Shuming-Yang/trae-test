...
> To generate the full coverage report locally, run:

```bash
python tools/gen_integration_coverage_report.py
```
...
> Run `python tools/gen_integration_coverage_report.py` to populate this section with actual test results.
...
<iframe
    ...
    title="Integration Test Code Coverage Report">
</iframe>
...
*This page is regenerated on every CI pipeline run to reflect the latest test results and code coverage.*