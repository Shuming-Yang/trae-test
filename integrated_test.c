...
/* ================================================================
 * IT-01: Number Mode Input Parsing
 * ================================================================ */

static int test_number_mode_irq0(void) {
    simulate_input("1");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-01-01: input '1' triggers IRQ0, pending cleared");
    return 0;
}

static int test_number_mode_irq31(void) {
    simulate_input("32");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-01-02: input '32' triggers IRQ31, pending cleared");
    return 0;
}
...
static int test_number_mode_zero(void) {
    irq_trigger(3);
    simulate_input("0");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-01-03: input '0' processes pending IRQ3");
    return 0;
}
...
static int test_number_mode_invalid_33(void) {
    ...
    simulate_input("33");
    IT_ASSERT_HEX_EQ(irq_get_pending(), before, "IT-01-04: input '33' should not change pending");
    return 0;
}
...
static int test_bmode_irq0(void) {
    simulate_input("b0");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-02-01: b0 triggers IRQ0, pending cleared");
    return 0;
}

static int test_bmode_irq5(void) {
    simulate_input("b5");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-02-02: b5 triggers IRQ5, pending cleared");
    return 0;
}

static int test_bmode_irq31(void) {
    simulate_input("b31");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-02-03: b31 triggers IRQ31, pending cleared");
    return 0;
}

static int test_bmode_uppercase(void) {
    simulate_input("B10");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-02-04: B10 triggers IRQ10, pending cleared");
    return 0;
}
...
static int test_bmode_invalid_32(void) {
    uint32_t before = irq_get_pending();
    simulate_input("b32");
    IT_ASSERT_HEX_EQ(irq_get_pending(), before, "IT-02-05: b32 should not change pending");
    return 0;
}
...
static int test_hmode_h1(void) {
    simulate_input("h1");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-03-01: h1 triggers IRQ0, pending cleared");
    return 0;
}

static int test_hmode_h3(void) {
    simulate_input("h3");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-03-02: h3 triggers IRQ0,1, pending cleared");
    return 0;
}

static int test_hmode_hFF(void) {
    simulate_input("hFF");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-03-03: hFF triggers IRQ0~7, pending cleared");
    return 0;
}

static int test_hmode_h80000000(void) {
    simulate_input("h80000000");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-03-04: h80000000 triggers IRQ31, pending cleared");
    return 0;
}

static int test_hmode_uppercase(void) {
    simulate_input("H0A");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-03-05: H0A triggers IRQ1,3, pending cleared");
    return 0;
}
...
static int test_hmode_invalid(void) {
    uint32_t before = irq_get_pending();
    simulate_input("hGG");
    IT_ASSERT_HEX_EQ(irq_get_pending(), before, "IT-03-06: hGG should not change pending");
    return 0;
}
...
static int test_accumulate_trigger_then_hmode(void) {
    irq_trigger(0);
    simulate_input("h6");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-04-01: trigger(0)+h6 processes IRQ0,1,2");
    return 0;
}
...
static int test_accumulate_multi_bmode(void) {
    ...
    simulate_input("b5");
    simulate_input("0");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-04-02: b10+b5+0 processes IRQ5,10");
    return 0;
}

static int test_priority_order(void) {
    simulate_input("h80000001");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-04-03: h80000001 processes IRQ0 before IRQ31");
    return 0;
}
...
static int test_tick_initial_zero(void) {
    IT_ASSERT_EQ(irq_get_tick(), 0, "IT-05-01: initial tick should be 0");
    return 0;
}
...
static int test_tick_irq0_increment(void) {
    ...
    irq_trigger(0);
    irq_process_all();
    IT_ASSERT_EQ(irq_get_tick(), before + 1, "IT-05-02: IRQ0 handler increments tick by 1");
    return 0;
}
...
static int test_tick_non_irq0_no_change(void) {
    ...
    irq_trigger(5);
    irq_process_all();
    IT_ASSERT_EQ(irq_get_tick(), before, "IT-05-03: non-IRQ0 should not change tick");
    return 0;
}
...
static int test_tick_multi_irq0(void) {
    ...
    irq_trigger(0);
    irq_process_all();
    IT_ASSERT_EQ(irq_get_tick(), before + 3, "IT-05-04: three separate IRQ0 triggers increment tick by 3");
    return 0;
}
...
static int test_exit_returns_one(void) {
    ...
    IT_ASSERT_EQ(result, 1, "IT-06-01: 'exit' should return 1");
    return 0;
}
...
static int test_empty_input_safe(void) {
    uint32_t before = irq_get_pending();
    simulate_input("");
    IT_ASSERT_HEX_EQ(irq_get_pending(), before, "IT-06-02: empty input should not crash");
    return 0;
}
...
static int test_garbage_input_safe(void) {
    uint32_t before = irq_get_pending();
    simulate_input("xyz");
    IT_ASSERT_HEX_EQ(irq_get_pending(), before, "IT-06-03: garbage input should not crash");
    return 0;
}
...
static int test_full_flow(void) {
    simulate_input("1");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-07-01: step1 '1' -> IRQ0 processed");

    simulate_input("b5");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-07-01: step2 'b5' -> IRQ5 processed");

    simulate_input("h3");
    IT_ASSERT_HEX_EQ(irq_get_pending(), 0, "IT-07-01: step3 'h3' -> IRQ0,1 processed");

    int result = simulate_input("exit");
    IT_ASSERT_EQ(result, 1, "IT-07-01: step4 'exit' -> returns 1");
...
int run_all_integrated_tests(void) {
    ...
    printf("[IT-01] Number Mode Input Parsing:\n");
